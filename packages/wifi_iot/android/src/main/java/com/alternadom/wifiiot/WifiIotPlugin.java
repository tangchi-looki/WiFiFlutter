...
public class WifiIotPlugin
  ...
  public void onMethodCall(MethodCall poCall, Result poResult) {
    switch (poCall.method) {
      ...
      case "forceWifiUsage":
        ...
        break;
      case "forceWifiUsageWithRetry":
        forceWifiUsageWithRetry(poCall, poResult);
        break;
      case "isEnabled":
        isEnabled(poResult);
        break;
  ...
  /// Enhanced forceWifiUsage with retry logic for problematic devices
  private void forceWifiUsageWithRetry(final MethodCall poCall, final Result poResult) {
    boolean useWifi = poCall.argument("useWifi");
    Integer retryCount = poCall.argument("retryCount");
    Integer retryDelay = poCall.argument("retryDelay");
    ...
    // Start retry logic in background thread
  ...
  private void performWifiForceWithRetry(int retryCount, int retryDelay, final Result poResult) {
    ...
    if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.LOLLIPOP || manager == null) {
      // For older versions, just return success
    ...
    for (int attempt = 0; attempt < retryCount; attempt++) {
      ...
      Log.d(WifiIotPlugin.class.getSimpleName(), "WiFi force attempt " + currentAttempt + "/" + retryCount);

      try {
        // Try the enhanced WiFi usage method
        boolean success = attemptWifiForce(manager);

        if (success) {
          Log.d(WifiIotPlugin.class.getSimpleName(), "WiFi force successful on attempt " + currentAttempt);
          new Handler(Looper.getMainLooper()).post(new Runnable() {
            @Override
            public void run() {
              poResult.success(true);
        ...
        if (attempt < retryCount - 1) {
          Log.d(WifiIotPlugin.class.getSimpleName(),
              "WiFi force attempt " + currentAttempt + " failed, retrying in " + retryDelay + "ms");
          Thread.sleep(retryDelay);
        }

      } catch (InterruptedException e) {
        Log.e(WifiIotPlugin.class.getSimpleName(), "WiFi force retry interrupted", e);
        break;
      } catch (Exception e) {
        Log.e(WifiIotPlugin.class.getSimpleName(), "WiFi force attempt " + currentAttempt + " failed with exception",
            e);
        if (attempt < retryCount - 1) {
          try {
            Thread.sleep(retryDelay);
    ...
    // All attempts failed
    Log.w(WifiIotPlugin.class.getSimpleName(), "All WiFi force attempts failed");
    new Handler(Looper.getMainLooper()).post(new Runnable() {
      @Override
      public void run() {
        poResult.success(false);
  ...
  /// Attempt to force WiFi usage with enhanced strategies
  private boolean attemptWifiForce(ConnectivityManager manager) {
    // Strategy 1: Use existing joined network if available
    if (joinedNetwork != null) {
      boolean success = selectNetwork(joinedNetwork, manager);
      Log.d(WifiIotPlugin.class.getSimpleName(), "Strategy 1 (joined network): " + success);
      if (success)
        return true;
    }

    // Strategy 2: Find and use current WiFi network
    Network currentWifiNetwork = getCurrentWifiNetwork(manager);
    if (currentWifiNetwork != null) {
      boolean success = selectNetwork(currentWifiNetwork, manager);
      Log.d(WifiIotPlugin.class.getSimpleName(), "Strategy 2 (current WiFi): " + success);
      if (success)
        return true;
    }

    // Strategy 3: Use synchronous network request (for retry scenarios)
    try {
      ...
      // For retry scenarios, try to get any available WiFi network immediately
      Network[] networks = manager.getAllNetworks();
      for (Network network : networks) {
        NetworkCapabilities capabilities = manager.getNetworkCapabilities(network);
        if (capabilities != null && capabilities.hasTransport(NetworkCapabilities.TRANSPORT_WIFI)) {
          boolean success = selectNetwork(network, manager);
          Log.d(WifiIotPlugin.class.getSimpleName(), "Strategy 3 (network scan): " + success);
          if (success)
            return true;
        }
      }
      ...
      Log.e(WifiIotPlugin.class.getSimpleName(), "Strategy 3 failed", e);
    }
